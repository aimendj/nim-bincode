...
proc personToBytes(p: Person): seq[byte] =
  var nameLenBytes = @[byte(p.name.len and 0xFF), 
...
proc bytesToPerson(data: seq[byte]): Person =
  ...
  if data.len >= offset + 4:
    let nameLen = (data[offset].uint32) or 
  ...
  if data.len >= offset + 4:
    result.age = (data[offset].uint32) or 
                 (data[offset + 1].uint32 shl 8) or
                 (data[offset + 2].uint32 shl 16) or
                 (data[offset + 3].uint32 shl 24)
    offset += 4
  ...
  if data.len >= offset + 4:
    let emailLen = (data[offset].uint32) or 
                   (data[offset + 1].uint32 shl 8) or
                   (data[offset + 2].uint32 shl 16) or
                   (data[offset + 3].uint32 shl 24)
    offset += 4
...
proc main() =
  ...
  echo "Match: ", (person.name == decoded.name and 
                   person.age == decoded.age and 
